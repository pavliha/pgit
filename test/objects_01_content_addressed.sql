...
SELECT isnt(grove.write_tree('t'), NULL, 'objects: write_tree returns a root');

SELECT ok(
  (SELECT count(*) FROM grove.nodes) > 1,
  'objects: a 5000 row table produces more than one node'
);

SELECT ok(
  EXISTS (SELECT 1 FROM grove.nodes WHERE hash = grove.write_tree('t')),
  'objects: the root is itself a stored node'
);

SELECT is(
  (SELECT sum(jsonb_array_length(entries))::bigint FROM grove.nodes WHERE level = 0),
  5000::bigint,
  'objects: level 0 entries account for every row exactly once'
);

CREATE TEMP TABLE before_rewrite AS SELECT count(*) AS n FROM grove.nodes;

SELECT grove.write_tree('t');

SELECT is(
  (SELECT count(*) FROM grove.nodes),
...
CREATE TEMP TABLE pre_update AS SELECT hash FROM grove.nodes WHERE level = 0;

UPDATE t SET name = 'changed' WHERE id = 2500;
SELECT grove.write_tree('t');

SELECT ok(
  (SELECT count(*) FROM grove.nodes WHERE level = 0
