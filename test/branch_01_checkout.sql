...
SELECT is(pgit.resolve('feature'), (SELECT sha FROM c1), 'branch: the new ref starts at HEAD');
...
SELECT throws_ok(
  $$ SELECT pgit.branch('feature') $$, NULL, NULL,
...
SELECT throws_ok(
  $$ SELECT pgit.checkout('feature') $$, NULL, NULL,
...
SELECT is(pgit.checkout('feature'), 0, 'AC-REF-03: checking out an identical commit writes no rows');
SELECT is(pgit.head(), 'feature', 'branch: checkout moves HEAD');

UPDATE t SET name = 'feature work' WHERE id BETWEEN 10 AND 12;
CREATE TEMP TABLE c2 AS SELECT pgit.commit('feature work', 'pavlo') AS sha;

SELECT is(
  pgit.checkout('main'), 3,
  'AC-REF-03: checkout writes only the differing rows, not the whole 200 row table'
);

SELECT is(
  pgit.tree_root('t'),
  (SELECT root_hash FROM pgit.trees WHERE commit_sha = (SELECT sha FROM c1) AND tbl = 't'),
  'branch: checking out main restores its content exactly'
);

SELECT is(
  pgit.checkout('feature'), 3,
  'AC-REF-03: switching back writes only the differing rows again'
);

SELECT is(
  pgit.tree_root('t'),
  (SELECT root_hash FROM pgit.trees WHERE commit_sha = (SELECT sha FROM c2) AND tbl = 't'),
  'branch: checking out feature restores its content exactly'
);

SELECT throws_ok(
  $$ SELECT pgit.delete_branch('feature') $$, NULL, NULL,
  'branch: deleting the checked out branch is refused'
);

SELECT pgit.branch('scratch', (SELECT sha FROM c1));
SELECT pgit.delete_branch('scratch');
