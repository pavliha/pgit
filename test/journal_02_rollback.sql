...
SELECT is(
  (SELECT after ->> 'name' FROM pgit.changes WHERE tbl = 't'),
...
SELECT pgit.track('untracked_after');
INSERT INTO untracked_after VALUES (1, 'a');
SELECT pgit.untrack('untracked_after');
INSERT INTO untracked_after VALUES (2, 'b');

SELECT is(
  (SELECT count(*) FROM pgit.changes WHERE tbl = 'untracked_after'),
