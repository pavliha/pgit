...
D="postgresql://postgres:grove@${GROVE_HOST:-localhost:5460}/grove_crash"
KILLS="${CRASH_KILLS:-12}"

psql "$ADMIN" -X -q -c "DROP DATABASE IF EXISTS grove_crash WITH (FORCE)" \
                -c "CREATE DATABASE grove_crash" >/dev/null
...
psql "$D" -X -q -At -c "SELECT grove.track('k1')" >/dev/null
...
psql "$D" -X -q -At -c "SELECT grove.branch('side', grove.resolve('main'))" >/dev/null
...
for i in $(seq 1 "$KILLS"); do
  case $((RANDOM % 5)) in
    0) WORK="UPDATE k1 SET n = n + 1 WHERE id % 7 = 0; SELECT grove.commit('c$i','main');" ;;
    1) WORK="DELETE FROM k1 WHERE id = $((100 + i)); SELECT grove.commit('d$i','main');" ;;
    2) WORK="INSERT INTO k1 VALUES ($((9000 + i)),'new',1); SELECT grove.commit('i$i','main');" ;;
    3) WORK="SELECT grove.repack();" ;;
    *) WORK="UPDATE k1 SET v = 'x$i' WHERE id % 11 = 0; SELECT grove.commit('u$i','main'); SELECT grove.revert(grove.resolve('main'));" ;;
  esac
...
is "crash: the branch still resolves and its history is reachable" \
   "$(psql "$D" -X -At -c "SELECT (grove.resolve('main') IS NOT NULL)")" "t"

is "crash: a commit still works afterwards" \
   "$(psql "$D" -X -At -c "UPDATE k1 SET n = n + 1 WHERE id = 1; SELECT (grove.commit('after crashes','main') IS NOT NULL)" | tail -1)" "t"
