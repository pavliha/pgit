...
CREATE TEMP TABLE journal_before AS SELECT count(*) AS n FROM grove.changes;
...
SELECT ok(
  (SELECT count(*) FROM grove.changes) > (SELECT n FROM journal_before),
  'grove journal is ENABLE ALWAYS, so it still records what replay actually wrote'
);

SELECT is(
  grove.tree_root('t'),
  (SELECT root_hash FROM grove.trees WHERE commit_sha = (SELECT sha FROM a) AND tbl = 't'),
  'AC-REPLAY-03: reverting a commit restores the parent content exactly, by root hash'
);

CREATE TEMP TABLE c AS SELECT grove.resolve('main') AS sha;
SELECT grove.revert((SELECT sha FROM c));
...
SELECT throws_ok(
  format($$ SELECT grove.revert(%L::bytea) $$, (SELECT sha FROM b)),
  NULL, NULL,
  'AC-REPLAY-04: revert refuses when a row it would touch has changed since'
);

CREATE TEMP TABLE journal_mark AS SELECT count(*) AS n FROM grove.changes;
CREATE TEMP TABLE reads AS
  SELECT count(*) AS c FROM grove.diff((SELECT sha FROM a), (SELECT sha FROM b));

SELECT is(
  (SELECT count(*) FROM grove.changes), (SELECT n FROM journal_mark),
