...
SRC="postgresql://postgres:pgit@${PGIT_HOST:-localhost:5460}/pgit_rt_src"
DST="postgresql://postgres:pgit@${PGIT_HOST:-localhost:5460}/pgit_rt_dst"
OPS="${RT_OPS:-120}"
ROWS="${RT_ROWS:-600}"
CHUNK="${RT_CHUNK:-8}"
ROUNDS="${RT_ROUNDS:-3}"
...
cleanup() {
  rm -f "$PACK"
  psql "$ADMIN" -X -q -c "DROP DATABASE IF EXISTS pgit_rt_src WITH (FORCE)" \
                  -c "DROP DATABASE IF EXISTS pgit_rt_dst WITH (FORCE)" >/dev/null 2>&1
}
trap cleanup EXIT
...
for round in $(seq 1 "$ROUNDS"); do
  ...
  psql "$ADMIN" -X -q -c "DROP DATABASE IF EXISTS pgit_rt_src WITH (FORCE)" \
                  -c "CREATE DATABASE pgit_rt_src" >/dev/null 2>&1
  ...
  psql "$SRC" -X -At -c "SELECT pgit.bundle(ARRAY['main'])" > "$PACK" 2>/dev/null
  if [ ! -s "$PACK" ]; then
    nok "roundtrip: seed $s produced no bundle"
    continue
  fi

  psql "$ADMIN" -X -q -c "DROP DATABASE IF EXISTS pgit_rt_dst WITH (FORCE)" \
  ...
  src_roots=$(psql "$SRC" -X -At -c "
    ...
    FROM pgit.trees t WHERE t.commit_sha = pgit.resolve('main')")
  ...
  is "roundtrip: seed $s clone is fsck clean" \
     "$(psql "$DST" -X -At -c 'SELECT count(*) FROM pgit.fsck()')" "0"
done
