...
SELECT 'sequential' AS shape, rows, chunks FROM grove.chunk_stats('seq_keys')
UNION ALL SELECT 'uuidv4', rows, chunks FROM grove.chunk_stats('uuid4_keys')
UNION ALL SELECT 'uuidv7', rows, chunks FROM grove.chunk_stats('uuid7_keys')
UNION ALL SELECT 'common-prefix', rows, chunks FROM grove.chunk_stats('prefix_keys');
...
SELECT grove.track('spin');
INSERT INTO spin VALUES (1, 'a'), (2, 'b');

SELECT ok(
  grove.is_boundary(convert_to('Id Col=#1:1|', 'UTF8'), grove.setting('chunk_target')::int),
  'AC-CANON-06: the fixture really is the pathological shape, first key on a boundary');

CREATE TEMP TABLE spin_c AS SELECT grove.commit('spin base', 'main') AS sha;

SELECT is(
  grove.write_tree('spin'),
  (SELECT root_hash FROM grove.trees WHERE commit_sha = (SELECT sha FROM spin_c) AND tbl = 'spin'),
  'AC-CANON-06: a level that cannot shrink still builds, and matches a rebuild');

UPDATE spin SET v = 'z' WHERE "Id Col" = 1;
SELECT grove.commit('spin update', 'main');

SELECT is(
  grove.write_tree('spin'),
  (SELECT root_hash FROM grove.trees WHERE commit_sha = grove.resolve('main') AND tbl = 'spin'),
...
SELECT grove.commit('spin churn', 'main');
