...
SELECT is(
  (SELECT sum(inserted + updated + deleted)::bigint FROM pgit.diff_stat((SELECT sha FROM a), (SELECT sha FROM b))),
...
SELECT is(
  (SELECT inserted || '/' || updated || '/' || deleted
   FROM pgit.diff_stat((SELECT sha FROM a), (SELECT sha FROM b)) WHERE tbl = 't1'),
  '1/2/1', 'AC-DIFF-06: stat splits t1 into one insert, two updates, one delete'
);

SELECT is(
  (SELECT added || '/' || removed FROM pgit.diff_numstat((SELECT sha FROM a), (SELECT sha FROM b)) WHERE tbl = 't1'),
  '3/3', 'AC-DIFF-06: numstat counts an update as both an addition and a removal'
);

SELECT is(
  (SELECT tables FROM pgit.diff_shortstat((SELECT sha FROM a), (SELECT sha FROM b))),
  2::bigint, 'AC-DIFF-06: shortstat counts both changed tables'
);

SELECT is(
  (SELECT string_agg(tbl, ',') FROM pgit.diff_name_only((SELECT sha FROM a), (SELECT sha FROM b))),
  't1,t2', 'AC-DIFF-06: name-only lists each changed table once'
);

SELECT is(
  (SELECT status FROM pgit.diff_name_status((SELECT sha FROM a), (SELECT sha FROM b)) WHERE tbl = 't1'),
  'M', 'AC-DIFF-06: an existing table is reported as modified'
);

SELECT is(
  (SELECT status FROM pgit.diff_name_status((SELECT sha FROM a), (SELECT sha FROM b)) WHERE tbl = 't2'),
  'A', 'AC-DIFF-06: a table that appears only in the newer commit is reported as added'
);

SELECT is(
  (SELECT count(*) FROM pgit.diff((SELECT sha FROM a), (SELECT sha FROM b), 't1')),
  (SELECT count(*) FROM pgit.diff((SELECT sha FROM a), (SELECT sha FROM b)) WHERE tbl = 't1'),
  'AC-DIFF-07: a table pathspec equals the full diff filtered by table'
);

SELECT is(
  (SELECT count(*) FROM pgit.diff((SELECT sha FROM a), (SELECT sha FROM b), 't1:5')),
  1::bigint, 'AC-DIFF-07: a row pathspec selects exactly one row'
);

SELECT is(
  (SELECT before ->> 'name' FROM pgit.diff((SELECT sha FROM a), (SELECT sha FROM b), 't1:5')),
  'row-5', 'AC-DIFF-07: the row pathspec selects the right row'
);

SELECT is(
  (SELECT count(*) FROM pgit.diff((SELECT sha FROM a), (SELECT sha FROM b), 't1.name')),
  3::bigint,
  'AC-DIFF-07: a column pathspec skips the row where only another column changed'
);

SELECT is(
  (SELECT before FROM pgit.diff((SELECT sha FROM a), (SELECT sha FROM b), 't1.name') WHERE after ->> 'name' = 'changed'),
  '{"name": "row-5"}'::jsonb,
  'AC-DIFF-07: a column pathspec reduces the images to that column'
);

SELECT is(
  (SELECT count(*) FROM pgit.diff((SELECT sha FROM a), (SELECT sha FROM b), 't1.note')),
  0::bigint,
...
SELECT pgit.track('t3');
INSERT INTO t3 VALUES (1, 'x', 9);
CREATE TEMP TABLE c AS SELECT pgit.commit('c', 'pavlo') AS sha;

SELECT throws_ok(
  format($$ SELECT count(*) FROM pgit.diff(%L::bytea, %L::bytea, 't3:1') $$,
