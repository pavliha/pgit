...
SELECT is(
  (SELECT count(*) FROM pgit.diff((SELECT sha FROM a), (SELECT sha FROM a))),
...
SELECT is(
  (SELECT op FROM pgit.diff((SELECT sha FROM a), (SELECT sha FROM b)) WHERE after ->> 'id' = '999'),
  'INSERT', 'diff: a new row is an INSERT'
);

SELECT is(
  (SELECT op FROM pgit.diff((SELECT sha FROM a), (SELECT sha FROM b)) WHERE before ->> 'id' = '200'),
  'DELETE', 'diff: a removed row is a DELETE'
);

SELECT is(
  (SELECT before ->> 'name' || '->' || (after ->> 'name')
   FROM pgit.diff((SELECT sha FROM a), (SELECT sha FROM b)) WHERE before ->> 'id' = '100'),
  'row-100->changed', 'diff: an updated row carries both images'
);

SELECT is(
  (SELECT count(*) FROM pgit.diff((SELECT sha FROM b), (SELECT sha FROM a))),
  3::bigint, 'AC-DIFF-03: the reverse diff reports the same three rows'
);

SELECT is(
  (SELECT op FROM pgit.diff((SELECT sha FROM b), (SELECT sha FROM a)) WHERE before ->> 'id' = '999'),
  'DELETE', 'AC-DIFF-03: the reverse diff inverts INSERT into DELETE'
);

SELECT is(
  pgit.apply_diff('snapshot_a', (SELECT sha FROM a), (SELECT sha FROM b), 't'),
...
SELECT ok(
  NOT EXISTS (
    SELECT 1 FROM pgit.diff((SELECT sha FROM a), (SELECT sha FROM b))
