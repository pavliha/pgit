...
CREATE TEMP TABLE base_c AS SELECT grove.commit('base', 'main') AS sha;

SELECT grove.branch('theirs', (SELECT sha FROM base_c));
...
CREATE TEMP TABLE ours_c AS SELECT grove.commit('ours', 'main') AS sha;

SELECT grove.checkout('theirs');
...
CREATE TEMP TABLE theirs_c AS SELECT grove.commit('theirs', 'theirs') AS sha;

CREATE TEMP TABLE roots AS
SELECT (SELECT x.root_hash FROM grove.trees x WHERE x.commit_sha = (SELECT sha FROM base_c)   AND x.tbl = 't') AS broot,
       (SELECT x.root_hash FROM grove.trees x WHERE x.commit_sha = (SELECT sha FROM ours_c)   AND x.tbl = 't') AS oroot,
       (SELECT x.root_hash FROM grove.trees x WHERE x.commit_sha = (SELECT sha FROM theirs_c) AND x.tbl = 't') AS troot;
...
FROM (SELECT d.k, d.before AS b, d.after AS o
      FROM roots, LATERAL grove.diff_tree(roots.broot, roots.oroot) d) dobj
FULL OUTER JOIN
     (SELECT d.k, d.before AS b, d.after AS t
      FROM roots, LATERAL grove.diff_tree(roots.broot, roots.troot) d) dthr
  ON dthr.k = dobj.k;
...
SELECT is(
  (SELECT count(*) FROM derived d, roots
   WHERE d.bimg IS DISTINCT FROM (SELECT l.v FROM grove.lookup(roots.broot, d.k) l)),
  0::bigint,
...
SELECT is(
  (SELECT count(*) FROM derived d, roots
   WHERE d.oimg IS DISTINCT FROM (SELECT l.v FROM grove.lookup(roots.oroot, d.k) l)),
  0::bigint,
...
SELECT is(
  (SELECT count(*) FROM derived d, roots
   WHERE d.timg IS DISTINCT FROM (SELECT l.v FROM grove.lookup(roots.troot, d.k) l)),
  0::bigint,
  'AC-MERGE-09: every theirs image matches, including keys only ours touched'
);

SELECT cmp_ok(
  (SELECT count(*) FROM grove.merge_plan((SELECT sha FROM base_c), (SELECT sha FROM ours_c),
...
SELECT is(
  (SELECT count(*) FROM grove.merge_plan((SELECT sha FROM base_c), (SELECT sha FROM ours_c),
                                        (SELECT sha FROM theirs_c)) mp
   WHERE mp.k IN (SELECT encode(grove.hash(convert_to('id=#' || length(g::text)::text || ':' || g || '|', 'UTF8')), 'hex')
