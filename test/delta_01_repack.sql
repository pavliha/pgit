...
CREATE TEMP TABLE before_trees   AS SELECT commit_sha, tbl, root_hash FROM grove.trees;
CREATE TEMP TABLE before_diff    AS SELECT * FROM grove.diff((SELECT sha FROM a), grove.resolve('main'));
CREATE TEMP TABLE before_blame   AS SELECT * FROM grove.blame('t', '2100');
CREATE TEMP TABLE before_size    AS
  SELECT sum(pg_column_size(entries) + COALESCE(pg_column_size(delta), 0) + COALESCE(pg_column_size(hashes), 0) + COALESCE(pg_column_size(keys), 0))::bigint AS sz,
         count(*) AS n FROM grove.nodes;

CREATE TEMP TABLE packed AS SELECT grove.repack() AS n;

SELECT cmp_ok((SELECT n FROM packed), '>', 100,
  'repack: more than a hundred node versions were deltified');

SELECT is(
  (SELECT count(*) FROM grove.nodes WHERE entries IS NULL), (SELECT n FROM packed)::bigint,
  'repack: exactly the deltified nodes now store a delta instead of full entries'
);

SELECT is(
  (SELECT count(*) FROM before_entries b
   WHERE b.e IS DISTINCT FROM grove.entries_of(b.hash)), 0::bigint,
...
SELECT is(
  (SELECT count(*) FROM (
     SELECT * FROM grove.blame('t', '2100') EXCEPT ALL SELECT * FROM before_blame) q), 0::bigint,
...
CREATE TEMP TABLE unpacked AS SELECT grove.unpack() AS n;
