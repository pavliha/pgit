...
SELECT grove.track('alpha');
SELECT grove.track('beta');
SELECT grove.commit('two tables');

ALTER TABLE alpha RENAME TO a2;
ALTER TABLE beta  RENAME TO b2;
SELECT grove.commit('both renamed');

SELECT is(
  (SELECT count(*)::int FROM grove.table_renames(grove.rev('HEAD~1'), grove.rev('HEAD'))), 2,
  'AC-RENAME-01: two tables renamed in one commit are both detected');

SELECT is(
  (SELECT r.new_tbl FROM grove.table_renames(grove.rev('HEAD~1'), grove.rev('HEAD')) r
   WHERE r.old_tbl = 'alpha'), 'a2',
  'AC-RENAME-01: and each is paired with its own successor rather than cross matched');

SELECT is(
  (SELECT r.kind FROM grove.table_renames(grove.rev('HEAD~1'), grove.rev('HEAD')) r
   WHERE r.old_tbl = 'beta'), 'identical',
  'AC-RENAME-01: an unchanged rename is reported as identical');

SELECT grove.untrack('b2');
DROP TABLE b2;
...
SELECT grove.track('unrelated');
SELECT grove.commit('dropped one, added another');

SELECT is(
  (SELECT count(*)::int FROM grove.table_renames(grove.rev('HEAD~1'), grove.rev('HEAD'))), 0,
...
SELECT grove.track('src');
SELECT grove.commit('src added');

ALTER TABLE src RENAME TO dst;
UPDATE dst SET v = 'changed ' || id WHERE id % 4 = 0;
SELECT grove.commit('renamed and a quarter changed');
...
SELECT is(
  (SELECT round(r.similarity, 2) FROM grove.table_renames(grove.rev('HEAD~1'), grove.rev('HEAD')) r
   WHERE r.old_tbl = 'src'), 0.75,
  'AC-RENAME-01: with the similarity measured exactly, a quarter changed is 0.75');

SELECT is(
  (SELECT count(*)::int FROM grove.table_renames(grove.rev('HEAD~1'), grove.rev('HEAD'), 0.95)), 0,
  'AC-RENAME-01: raising the threshold above that similarity reports nothing');

SELECT is(
  (SELECT count(*)::int FROM grove.table_renames(grove.rev('HEAD~1'), grove.rev('HEAD'), 0.50)), 1,
