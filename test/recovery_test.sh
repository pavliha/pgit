...
NAME="${RECOVERY_CONTAINER:-grove-recovery}"
PORT="${RECOVERY_PORT:-5463}"
PG="${RECOVERY_PG:-18}"
KILLS="${RECOVERY_KILLS:-4}"
H="localhost:$PORT"
ADMIN="postgresql://postgres:grove@$H/postgres"
D="postgresql://postgres:grove@$H/grove_rec"
...
docker run -d --name "$NAME" -e POSTGRES_PASSWORD=grove -e POSTGRES_DB=grove_test \
...
setup() {
  psql "$ADMIN" -X -q -v ON_ERROR_STOP=1 -c "CREATE DATABASE grove_rec" || return 1
  ...
  psql "$D" -X -q -v ON_ERROR_STOP=1 \
    ...
    -c "SELECT grove.track('r')" \
    -c "INSERT INTO r SELECT g,'v'||g,0 FROM generate_series(1,20000) g" \
    -c "SELECT grove.commit('base','main')" >/dev/null || return 1
}

if ! setup; then nok "recovery: could not build the fixture database"; suite_end RECOVERY 1; fi

BEFORE=$(psql "$D" -X -At -c "SELECT encode(grove.resolve('main'),'hex')")

recovered=0
violations=0
for i in $(seq 1 "$KILLS"); do
  case $((i % 4)) in
    0) WORK="UPDATE r SET n = n + $i WHERE id % 3 = 0; SELECT grove.commit('k$i','main');" ;;
    1) WORK="UPDATE r SET v = 'k$i' WHERE id % 5 = 0; SELECT grove.commit('k$i','main'); SELECT grove.repack();" ;;
    2) WORK="DELETE FROM r WHERE id = $((500 + i)); SELECT grove.commit('k$i','main');" ;;
    *) WORK="INSERT INTO r SELECT g,'n$i',1 FROM generate_series($((30000 + i*100)), $((30000 + i*100 + 200))) g; SELECT grove.commit('k$i','main');" ;;
  esac
  ...
  BAD=$(psql "$D" -X -At -c "
    SELECT (SELECT count(*) FROM grove.fsck())
         + (SELECT count(*) FROM grove.trees t
            WHERE t.commit_sha = grove.resolve('main')
              AND grove.write_tree(t.tbl::regclass) IS DISTINCT FROM t.root_hash)" 2>&1)
  if [ "$BAD" != "0" ]; then
    violations=$((violations + 1))
...
AFTER=$(psql "$D" -X -At -c "SELECT encode(grove.resolve('main'),'hex')")
isnt "recovery: committed work before the crashes was not lost" "$AFTER" ""

is "recovery: the branch still resolves to a real commit" \
   "$(psql "$D" -X -At -c "SELECT count(*) FROM grove.commits WHERE sha = grove.resolve('main')")" "1"

psql "$D" -X -q -c "UPDATE r SET n = n + 1 WHERE id = 1" >/dev/null
is "recovery: grove still commits after all of it" \
   "$(psql "$D" -X -At -c "SELECT (grove.commit('after recovery','main') IS NOT NULL)" | tail -1)" "t"

is "recovery: and that commit's tree matches a rebuild" \
   "$(psql "$D" -X -At -c "SELECT grove.write_tree('r') = (SELECT root_hash FROM grove.trees WHERE commit_sha = grove.resolve('main') AND tbl = 'r')")" "t"
