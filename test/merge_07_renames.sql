...
CREATE TEMP TABLE b AS SELECT pgit.commit('base','p') AS sha;
SELECT pgit.branch('feature');

UPDATE t SET name='beta' WHERE id=1;
CREATE TEMP TABLE mn AS SELECT pgit.commit('main edits the name','p') AS sha;

SELECT pgit.checkout('feature');
UPDATE t SET id=100 WHERE id=1;
CREATE TEMP TABLE ft AS SELECT pgit.commit('feature changes the key','p') AS sha;

SELECT is(
  (SELECT count(*) FROM pgit.rename_pairs(
     (SELECT root_hash FROM pgit.trees WHERE commit_sha=(SELECT sha FROM b) AND tbl='t'),
     (SELECT root_hash FROM pgit.trees WHERE commit_sha=(SELECT sha FROM ft) AND tbl='t'),
     ARRAY['id'])), 1::bigint,
  'renames: a primary key change is detected as one rename, not a delete plus an insert');

SELECT is(
  (SELECT round(sim,2) FROM pgit.rename_pairs(
     (SELECT root_hash FROM pgit.trees WHERE commit_sha=(SELECT sha FROM b) AND tbl='t'),
     (SELECT root_hash FROM pgit.trees WHERE commit_sha=(SELECT sha FROM ft) AND tbl='t'),
     ARRAY['id'])), 1.00,
...
SELECT pgit.commit('feature edits row two','p');

SELECT pgit.checkout('side');
DELETE FROM t WHERE id=2;
INSERT INTO t VALUES (200,'utterly different content',777);
SELECT pgit.commit('side replaces row two','p');

SELECT is(pgit.merge('feature'), 1,
  'renames: a dissimilar delete and insert is not mistaken for a rename');

SELECT is(
  (SELECT count(*) FROM pgit.conflicts WHERE NOT resolved), 1::bigint,
