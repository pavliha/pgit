...
SELECT is(
  (SELECT count(*) FROM grove.changes WHERE tbl = 't' AND op = 'INSERT'), 100::bigint,
  'journal: one multi row INSERT statement journals every row'
);

SELECT is(
  (SELECT count(DISTINCT txid) FROM grove.changes WHERE tbl = 't'), 1::bigint,
...
SELECT is(
  (SELECT count(*) FROM grove.changes WHERE tbl = 't' AND op = 'UPDATE'), 40::bigint,
  'journal: one multi row UPDATE journals each row with both images'
);

SELECT is(
  (SELECT before ->> 'hits' || '->' || (after ->> 'hits')
   FROM grove.changes WHERE tbl = 't' AND pk = '{"id": 5}'::jsonb),
...
SELECT is(
  (SELECT string_agg(op, ',' ORDER BY op) FROM grove.changes WHERE tbl = 't'),
  'DELETE,INSERT',
...
SELECT is(
  (SELECT count(*) FROM grove.changes WHERE tbl = 't' AND op = 'DELETE'), 20::bigint,
