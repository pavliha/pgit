...
SELECT grove.track('ux');
SELECT grove.commit('initial catalogue', NULL, now());

SELECT isnt(
  (SELECT author FROM grove.log() ORDER BY depth LIMIT 1), NULL,
  'AC-UX-01: a commit records an author without the user configuring one first');

SELECT is(
  (SELECT count(*)::int FROM grove.blame('ux', '1')), 3,
  'AC-UX-02: blame attributes every column of a row that existed before track()');

SELECT is(
  (SELECT b.commit_sha FROM grove.blame('ux', '1') b WHERE b.col = 'name'),
  grove.resolve('main'),
  'AC-UX-02: and credits them to the commit that first recorded the row');

SELECT throws_ok(
  $$ SELECT grove.commit('nothing changed at all') $$,
  NULL,
  NULL,
  'AC-UX-03: committing an unchanged working tree is refused, not silently recorded');

SELECT is(
  (SELECT count(*)::int FROM grove.commits), 1,
  'AC-UX-03: so a repeating job cannot fill history with empty commits');

SELECT isnt(
  grove.commit('deliberate marker', NULL, now(), true), NULL,
  'AC-UX-03: unless the caller asks for an empty commit on purpose');

UPDATE ux SET price = 27.50 WHERE id = 1;
SELECT grove.commit('raise the price');

SELECT is(
  (SELECT b.value FROM grove.blame('ux', '1') b WHERE b.col = 'price'), '27.50'::jsonb,
  'AC-UX-02: blame follows a later change to a single column');

SELECT is(
  (SELECT b.commit_sha FROM grove.blame('ux', '1') b WHERE b.col = 'price'),
  grove.resolve('main'),
  'AC-UX-02: and moves that column to the newer commit while the rest stay put');

CREATE TEMP TABLE reverted AS SELECT grove.revert(grove.resolve('main')) AS sha;

SELECT is(
  (SELECT sha FROM reverted), grove.resolve('main'),
...
SELECT is(
  (SELECT message FROM grove.log() ORDER BY depth LIMIT 1), 'Revert "raise the price"',
