...
CREATE TEMP TABLE c0 AS SELECT grove.commit('base', 'p') AS sha;

CREATE TEMP TABLE roots0 AS
SELECT tbl, root_hash FROM grove.trees WHERE commit_sha = (SELECT sha FROM c0);

SELECT cmp_ok(
  (SELECT count(*) FROM grove.nodes_at_level((SELECT root_hash FROM roots0 WHERE tbl = 'big'), 0)),
  '>', 100::bigint,
  'untouched: the big table really is many chunks, so rebuilding it would be expensive');

UPDATE small SET v = 'changed' WHERE id = 1;
CREATE TEMP TABLE c1 AS SELECT grove.commit('touch only the small table', 'p') AS sha;

SELECT is(
  (SELECT root_hash FROM grove.trees WHERE commit_sha = (SELECT sha FROM c1) AND tbl = 'big'),
  (SELECT root_hash FROM roots0 WHERE tbl = 'big'),
  'untouched: the untouched table keeps exactly the root the parent recorded');

SELECT isnt(
  (SELECT root_hash FROM grove.trees WHERE commit_sha = (SELECT sha FROM c1) AND tbl = 'small'),
  (SELECT root_hash FROM roots0 WHERE tbl = 'small'),
  'untouched: while the table that did change got a new root');

SELECT is(
  grove.write_tree('big'),
  (SELECT root_hash FROM grove.trees WHERE commit_sha = (SELECT sha FROM c1) AND tbl = 'big'),
  'untouched: and that reused root is what a full rebuild produces anyway');

SELECT is(grove.is_dirty(), false, 'untouched: the working tree is clean after the commit');

UPDATE big SET v = 'now changed' WHERE id = 12345;
CREATE TEMP TABLE c2 AS SELECT grove.commit('touch the big table', 'p') AS sha;

SELECT isnt(
  (SELECT root_hash FROM grove.trees WHERE commit_sha = (SELECT sha FROM c2) AND tbl = 'big'),
  (SELECT root_hash FROM roots0 WHERE tbl = 'big'),
...
SELECT is(
  (SELECT count(*) FROM grove.diff((SELECT sha FROM c1), (SELECT sha FROM c2))), 1::bigint,
  'untouched: the diff across that commit reports exactly the one changed row');

SELECT is(
  (SELECT count(*) FROM grove.diff((SELECT sha FROM c0), (SELECT sha FROM c1), 'big')), 0::bigint,
...
BEGIN
  ...
  t0 := clock_timestamp();
  PERFORM grove.commit('timing: touch only the small table', 'p');
  INSERT INTO t_ms VALUES ('commit', extract(epoch FROM clock_timestamp() - t0) * 1000);

  t0 := clock_timestamp();
  PERFORM grove.write_tree('big');
