...
SELECT grove.commit('wide', 'p');

SELECT cmp_ok(
  (SELECT count(*) FROM grove.nodes_at_level(
     (SELECT root_hash FROM grove.trees WHERE commit_sha = grove.resolve('main') AND tbl = 't'), 0)),
  '>', 1::bigint,
  'collapse: the starting tree really has more than one leaf, so the shrink is a real shrink');

DELETE FROM t WHERE id > 20;
SELECT grove.commit('shrunk to a single chunk', 'p');

CREATE TEMP TABLE after AS
SELECT root_hash AS h FROM grove.trees WHERE commit_sha = grove.resolve('main') AND tbl = 't';

SELECT is(
  (SELECT count(*) FROM grove.nodes_at_level((SELECT h FROM after), 0)), 1::bigint,
  'collapse: the table now fits in a single chunk');

SELECT is(
  grove.write_tree('t'), (SELECT h FROM after),
  'collapse: the incrementally spliced root equals a full rebuild of the same rows');

SELECT is(
  grove.node_level((SELECT h FROM after)), 0,
...
SELECT is(
  (SELECT count(*) FROM grove.diff(grove.rev('main~1'), grove.resolve('main'))), 3980::bigint,
  'collapse: the diff still reports every deleted row');

DELETE FROM t;
SELECT grove.commit('emptied', 'p');
