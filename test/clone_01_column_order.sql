...
SELECT pgit.track('ord_src');
SELECT pgit.commit('a table whose columns are not in alphabetical order');

SELECT is(
  (SELECT string_agg(e.value ->> 'name', ',' ORDER BY e.ordinality)
   FROM pgit.schemas s, jsonb_array_elements(s.columns) WITH ORDINALITY e
   WHERE s.commit_sha = pgit.resolve('main') AND s.tbl = 'ord_src'),
  'zebra,alpha,middle',
