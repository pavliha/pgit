...
BEGIN
  FOR i IN 1..50 LOOP
    ...
    EXECUTE format('SELECT pgit.track(''t%s'')', i);
  END LOOP;
END $$;

CREATE TEMP TABLE a AS SELECT pgit.commit('fifty tables', 'bench') AS sha;

UPDATE t7 SET hits = 1 WHERE id <= 3;
CREATE TEMP TABLE b AS SELECT pgit.commit('touch one table', 'bench') AS sha;

SELECT is(
  (SELECT count(*) FROM pgit.trees WHERE commit_sha = (SELECT sha FROM a)), 50::bigint,
  'AC-PERF-03: all fifty tables are tracked and recorded'
);

SELECT is(
  (SELECT count(*) FROM pgit.trees ta
   JOIN pgit.trees tb ON tb.tbl = ta.tbl AND tb.commit_sha = (SELECT sha FROM b)
...
SELECT is(
  (SELECT count(*) FROM pgit.diff_stat((SELECT sha FROM a), (SELECT sha FROM b))), 1::bigint,
  'AC-PERF-03: stat reports exactly the one changed table'
);

CREATE TEMP TABLE blocks_one AS
SELECT pg_stat_get_xact_blocks_fetched('pgit.nodes'::regclass) AS b0;

SELECT count(*) FROM pgit.diff_stat((SELECT sha FROM a), (SELECT sha FROM b));

CREATE TEMP TABLE blocks_after_one AS
SELECT pg_stat_get_xact_blocks_fetched('pgit.nodes'::regclass) AS b1;
...
CREATE TEMP TABLE c AS SELECT pgit.commit('touch all fifty', 'bench') AS sha;

CREATE TEMP TABLE blocks_two AS
SELECT pg_stat_get_xact_blocks_fetched('pgit.nodes'::regclass) AS b0;

SELECT count(*) FROM pgit.diff_stat((SELECT sha FROM b), (SELECT sha FROM c));

CREATE TEMP TABLE blocks_after_two AS
SELECT pg_stat_get_xact_blocks_fetched('pgit.nodes'::regclass) AS b1;

SELECT is(
  (SELECT count(*) FROM pgit.diff_stat((SELECT sha FROM b), (SELECT sha FROM c))), 50::bigint,
