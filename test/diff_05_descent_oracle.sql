...
LANGUAGE sql STABLE AS $fn$
  SELECT COALESCE(la.k, lb.k),
         CASE WHEN la.k IS NULL THEN 'INSERT'
              ...
              ELSE 'UPDATE' END,
         la.v, lb.v
  FROM grove.leaves(a) la FULL OUTER JOIN grove.leaves(b) lb ON la.k = lb.k
...
LANGUAGE sql STABLE AS $fn$
  SELECT (SELECT count(*) FROM (
            SELECT * FROM grove.diff_tree(a, b)
            EXCEPT ALL SELECT * FROM brute_diff(a, b)) q)
       + (SELECT count(*) FROM (
            SELECT * FROM brute_diff(a, b)
            EXCEPT ALL SELECT * FROM grove.diff_tree(a, b)) q)
$fn$;
...
SELECT cmp_ok((SELECT count(*) FROM grove.nodes_at_level((SELECT root FROM c0), 0)), '>', 20::bigint,
...
SELECT grove.commit('interleave', 'p');
...
SELECT is((SELECT count(*) FROM grove.diff_tree((SELECT root FROM c0), (SELECT root FROM c1))),
  5000::bigint, 'oracle: and it reports every inserted row');

UPDATE t SET v = v || '-x' WHERE id % 730 = 0;
SELECT grove.commit('scattered update', 'p');
...
SELECT grove.commit('range delete', 'p');
...
SELECT is((SELECT count(*) FROM grove.diff_tree((SELECT root FROM c4), (SELECT root FROM c4))),
