...
CREATE TEMP TABLE g0 AS SELECT pgit.commit('generated base', 'p') AS sha;
...
SELECT is(pgit.is_dirty(), false, 'generated: the tree is clean after replaying over a generated column');

DELETE FROM g WHERE id = 2;
SELECT pgit.commit('drop one', 'p');
SELECT pgit.reset(encode((SELECT sha FROM g0), 'hex'), 'hard');
...
SELECT pgit.track('ident');
INSERT INTO ident (note) VALUES ('first'), ('second');
CREATE TEMP TABLE i0 AS SELECT pgit.commit('identity base', 'p') AS sha;

DELETE FROM ident;
SELECT pgit.commit('cleared', 'p');
SELECT is((SELECT count(*) FROM ident), 0::bigint, 'identity: the rows really were removed');

SELECT pgit.reset(encode((SELECT sha FROM i0), 'hex'), 'hard');
...
SELECT is(pgit.is_dirty(), false, 'identity: and the restored table matches the commit exactly');
