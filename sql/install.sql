...
LANGUAGE sql STABLE AS $$
  SELECT COALESCE(NULLIF(current_setting('pgit.actor', true), ''), current_user)
$$;
...
BEGIN
  ...
  PERFORM pgit.emit('track', started, jsonb_build_object('table', target::text));
...
BEGIN
  ...
  EXECUTE format('DROP TRIGGER IF EXISTS pgit_journal_del ON %s', target::text);
  EXECUTE format('DROP TRIGGER IF EXISTS pgit_journal_truncate ON %s', target::text);
  PERFORM pgit.emit('untrack', started, jsonb_build_object('table', target::text));
...
CREATE TABLE IF NOT EXISTS pgit.events (
...
CREATE INDEX IF NOT EXISTS events_at_idx ON pgit.events (at DESC);
CREATE INDEX IF NOT EXISTS events_verb_idx ON pgit.events (verb, at DESC);

CREATE OR REPLACE FUNCTION pgit.emit(verb text, started timestamptz, detail jsonb DEFAULT '{}'::jsonb,
...
DECLARE
  ...
  wide jsonb;
BEGIN
  IF pgit.setting('log_server') = 'on' THEN
    wide := jsonb_build_object('verb', verb, 'ok', ok, 'actor', who, 'branch', br,
                               'duration_ms', ms, 'txid', txid_current()) || detail;
    RAISE LOG 'pgit %', wide::text;
  END IF;

  IF pgit.setting('log_events') = 'on' THEN
    INSERT INTO pgit.events (verb, ok, actor, branch, duration_ms, detail)
...
DROP FUNCTION IF EXISTS pgit.commit(text, text, timestamptz);
...
BEGIN
  ...
  IF NOT allow_empty AND parent IS NOT NULL AND roots =
     (SELECT COALESCE(jsonb_object_agg(t.tbl, encode(t.root_hash, 'hex')), '{}'::jsonb)
      FROM pgit.trees t WHERE t.commit_sha = parent)
     AND NOT EXISTS (
       SELECT 1 FROM pgit.tracked tr
       WHERE pgit.schema_fingerprint(tr.tbl) IS DISTINCT FROM
             (SELECT sc.fingerprint FROM pgit.schemas sc
              WHERE sc.commit_sha = parent AND sc.tbl = tr.tbl::text))
  THEN
    RAISE EXCEPTION 'pgit: nothing to commit, every tracked table already matches %',
      pgit.short_sha(parent)
  ...
  PERFORM pgit.emit('commit', started, jsonb_build_object(
    'sha', pgit.short_sha(new_sha), 'parent', pgit.short_sha(parent),
...
DROP FUNCTION IF EXISTS pgit.revert(bytea);
CREATE OR REPLACE FUNCTION pgit.revert(target_sha bytea, msg text DEFAULT NULL) RETURNS bytea
LANGUAGE plpgsql AS $$
DECLARE
  parent    bytea;
  r         record;
...
BEGIN
  ...
  new_sha := pgit.commit(
    COALESCE(msg, 'Revert "' || COALESCE(orig_msg, pgit.short_sha(target_sha)) || '"'),
    NULL, now(), false);

  PERFORM pgit.emit('revert', started, jsonb_build_object(
    'reverted', pgit.short_sha(target_sha), 'sha', pgit.short_sha(new_sha), 'rows', applied));
...
BEGIN
  ...
  PERFORM pgit.emit('branch', started, jsonb_build_object(
    'name', branch_name, 'at', pgit.short_sha(target)));
...
BEGIN
  ...
  PERFORM pgit.emit('checkout', started, jsonb_build_object(
    'to', branch_name, 'from', pgit.short_sha(cur), 'rows', applied, 'forced', force));
...
BEGIN
  ...
  PERFORM pgit.emit('delete_branch', started, jsonb_build_object(
    'name', branch_name, 'was', pgit.short_sha(pgit.resolve(branch_name))));
...
DROP FUNCTION IF EXISTS pgit.blame(text, text);
...
BEGIN
  ...
  SELECT (pgit.pk_columns(tbl_name::regclass))[1] INTO pkcol;

  EXECUTE format('SELECT encode(convert_to(%s, %L), %L) FROM %s WHERE %I::text = %L LIMIT 1',
                 pgit.pk_canon_expr(tbl_name::regclass), 'utf8', 'hex',
                 tbl_name::regclass::text, pkcol, key_value)
  INTO key_hex;

  IF key_hex IS NOT NULL THEN
    FOR c IN SELECT l.sha, l.at, l.author FROM pgit.log() l ORDER BY l.depth DESC LOOP
      SELECT lo.v INTO img
      FROM pgit.lookup((SELECT t.root_hash FROM pgit.trees t
...
BEGIN
  ...
  IF n > 0 THEN
    PERFORM pgit.emit('merge', started, jsonb_build_object(
...
BEGIN
  ...
  PERFORM pgit.emit('cherry_pick', started, jsonb_build_object(
    'from', pgit.short_sha(target_sha), 'sha', pgit.short_sha(new_sha),
    'onto', pgit.short_sha(ours)));
...
BEGIN
  ...
  IF base = onto THEN
    PERFORM pgit.emit('rebase', started, jsonb_build_object(
      'onto', onto_branch, 'branch', cur_branch, 'was', pgit.short_sha(ours),
      'now', pgit.short_sha(ours), 'rewritten', false, 'noop', true));
    RETURN 0;
  END IF;
  ...
  FOR r IN SELECT sha FROM pgit_replay ORDER BY depth DESC LOOP
    ...
    IF n > 0 THEN
      PERFORM pgit.emit('rebase', started, jsonb_build_object(
        'onto', onto_branch, 'branch', cur_branch, 'was', pgit.short_sha(ours),
        'stopped_at', pgit.short_sha(r.sha), 'conflicts', n, 'finished', false), false);
      RETURN n;
    END IF;
  END LOOP;
  ...
  PERFORM pgit.emit('rebase', started, jsonb_build_object(
    'onto', onto_branch, 'branch', cur_branch, 'was', pgit.short_sha(ours),
    'now', pgit.short_sha(pgit.resolve(cur_branch)),
    'rewritten', NOT EXISTS (SELECT 1 FROM pgit.ancestors(pgit.resolve(cur_branch)) a
                             WHERE a.a = ours)));
...
BEGIN
  ...
  PERFORM pgit.emit('rebase_abort', started, jsonb_build_object(
    'branch', st.branch, 'back_to', pgit.short_sha(st.original_sha)), false);
...
BEGIN
  ...
  FOR chunk_rec IN SELECT o.rn, o.k, o.h FROM pgit_old o JOIN pgit_hit x ON x.rn = o.rn LOOP
    ...
    FROM pgit.node_raw(decode(chunk_rec.h, 'hex')) r;
...
BEGIN
  ...
  IF lvl = want THEN
    RETURN QUERY
      SELECT (SELECT r.keys[1] FROM pgit.node_raw(root) r),
             encode(root, 'hex');
    RETURN;
  END IF;
...
DECLARE
  ...
  parts        jsonb;
  started      timestamptz := clock_timestamp();
  bytes_before bigint := pg_total_relation_size('pgit.nodes');
BEGIN
  FOR grp IN
  ...
  PERFORM pgit.log_rotate();

  PERFORM pgit.emit('repack', started, jsonb_build_object(
    'packed', packed, 'max_depth', max_depth,
    'nodes', (SELECT count(*) FROM pgit.nodes),
    'bytes_before', bytes_before,
    'bytes_after', pg_total_relation_size('pgit.nodes')));
...
BEGIN
  ...
  PERFORM pgit.emit('resolve_conflict', started, jsonb_build_object(
...
BEGIN
  ...
  PERFORM pgit.emit('merge_abort', started, jsonb_build_object(
    'merge_id', mid,
    'discarded_conflicts', (SELECT count(*) FROM pgit.conflicts WHERE merge_id = mid)), false);
...
BEGIN
  ...
  PERFORM pgit.emit('merge_finish', started, jsonb_build_object(
    'merge_id', mid, 'sha', pgit.short_sha(new_sha)));
...
BEGIN
  ...
  PERFORM pgit.emit('reset', started, jsonb_build_object(
    'spec', spec, 'mode', mode, 'from', pgit.short_sha(cur), 'to', pgit.short_sha(tgt),
    'rows', applied));
...
DECLARE
  ...
  started timestamptz := clock_timestamp();
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pgit.remotes r WHERE r.name = remote_name) THEN
    RAISE EXCEPTION 'pgit: no remote named %', remote_name
      USING HINT = 'add it first: SELECT pgit.remote_add(' || quote_literal(remote_name)
  ...
  PERFORM pgit.emit('fetch', started, jsonb_build_object('remote', remote_name, 'commits', n));
...
BEGIN
  ...
  PERFORM pgit.emit('receive', started, jsonb_build_object('commits', n, 'forced', force));
...
BEGIN
  ...
  PERFORM pgit.emit('tag', started, jsonb_build_object(
    'name', tag_name, 'at', pgit.short_sha(target), 'forced', force));
...
DECLARE
  started timestamptz := clock_timestamp();
  was     bytea       := (SELECT t.sha FROM pgit.tags t WHERE t.name = tag_name);
BEGIN
  DELETE FROM pgit.tags WHERE name = tag_name;
  PERFORM pgit.emit('tag_delete', started, jsonb_build_object(
    'name', tag_name, 'was', pgit.short_sha(was)));
END $$;
...
BEGIN
  ...
  SET CONSTRAINTS ALL IMMEDIATE;
  PERFORM pgit.replay_end(guard);
  PERFORM pgit.emit('restore', started, jsonb_build_object(
...
BEGIN
  ...
  PERFORM pgit.emit('stash_push', started, jsonb_build_object('slot', slot, 'msg', msg));
...
BEGIN
  ...
  PERFORM pgit.emit('stash_pop', started, jsonb_build_object('slot', pick, 'rows', n));
...
BEGIN
  ...
  PERFORM pgit.emit('bisect_start', started, jsonb_build_object(
...
BEGIN
  ...
  PERFORM pgit.emit('prune', started, jsonb_build_object(
    'commits_removed', n, 'before', before_at,
    'commits_left', (SELECT count(*) FROM pgit.commits),
    'nodes_left', (SELECT count(*) FROM pgit.nodes)));
...
BEGIN
  ...
  SELECT string_agg(format('%I %s', e.value ->> 'name', pgit.checked_type(e.value ->> 'type')), ', '
...
BEGIN
  ...
  PERFORM pgit.emit('clone_from', started, jsonb_build_object(
    'branch', branch, 'tables', made, 'tip', pgit.short_sha(tip)));
...
BEGIN
  IF target IS NULL THEN RAISE EXCEPTION 'pgit: cannot resolve %', spec; END IF;
  PERFORM pgit.emit('note_add', started, jsonb_build_object(
    'spec', spec, 'sha', pgit.short_sha(target)));
...
BEGIN
  ...
  PERFORM pgit.emit('merge_octopus', started, jsonb_build_object(
    'branches', to_jsonb(branch_names), 'sha', pgit.short_sha(new_sha),
    'parents', array_length(all_h, 1), 'onto', pgit.short_sha(ours)));
...
LANGUAGE sql STABLE AS $$
  ...
  UNION ALL SELECT 'merge in progress',
                   COALESCE((SELECT string_agg(m.branch || ' <- ' || pgit.short_sha(m.theirs_sha), ', ')
                             FROM pgit.merges m), 'no'),
                   EXISTS (SELECT 1 FROM pgit.merges)
  UNION ALL SELECT 'conflicts awaiting resolution',
                   (SELECT count(*)::text FROM pgit.conflicts WHERE NOT resolved),
                   EXISTS (SELECT 1 FROM pgit.conflicts WHERE NOT resolved)
...
CREATE OR REPLACE FUNCTION pgit.metrics() RETURNS TABLE (metric text, value numeric)
LANGUAGE sql STABLE AS $$
  WITH n AS (
  ...
  SELECT 'pgit_commits_total', count(*)::numeric FROM pgit.commits
  UNION ALL SELECT 'pgit_branches', count(*)::numeric FROM pgit.refs
  UNION ALL SELECT 'pgit_tracked_tables', count(*)::numeric FROM pgit.tracked
  UNION ALL SELECT 'pgit_nodes_total', n.total::numeric FROM n
  UNION ALL SELECT 'pgit_nodes_packed', n.packed::numeric FROM n
  UNION ALL SELECT 'pgit_node_bytes', n.bytes::numeric FROM n
  UNION ALL SELECT 'pgit_journal_rows', j.total::numeric FROM j
  UNION ALL SELECT 'pgit_journal_pending', j.pending::numeric FROM j
  UNION ALL SELECT 'pgit_journal_bytes', j.bytes::numeric FROM j
  UNION ALL SELECT 'pgit_merges_open', count(*)::numeric FROM pgit.merges
  UNION ALL SELECT 'pgit_conflicts_unresolved', count(*)::numeric FROM pgit.conflicts WHERE NOT resolved
  UNION ALL SELECT 'pgit_events_total', e.total::numeric FROM e
  UNION ALL SELECT 'pgit_events_failed', e.failed::numeric FROM e
  UNION ALL SELECT 'pgit_commit_ms_p50', COALESCE(e.commit_p50, 0) FROM e
  UNION ALL SELECT 'pgit_commit_ms_p95', COALESCE(e.commit_p95, 0) FROM e
  UNION ALL SELECT 'pgit_commit_ms_max', COALESCE(e.commit_max, 0) FROM e
  UNION ALL SELECT 'pgit_seconds_since_last_commit',
                   COALESCE(round(extract(epoch FROM now() - max(at))), -1) FROM pgit.commits
$$;

CREATE OR REPLACE FUNCTION pgit.log_rotate() RETURNS int
LANGUAGE plpgsql AS $$
DECLARE
  n int;
BEGIN
  DELETE FROM pgit.events
  WHERE at < now() - (pgit.setting('log_retain_days') || ' days')::interval;
  GET DIAGNOSTICS n = ROW_COUNT;
  RETURN n;
END $$;
