...
QDB="${QDB:-grove_quick}"
D="postgresql://postgres:grove@${GROVE_HOST:-localhost:5460}/$QDB"
...
BEGIN
  ...
  t0 := clock_timestamp(); PERFORM grove.commit('sc','p');
  ...
  FOR n IN 1..$NIGHTS LOOP
    ...
    t0 := clock_timestamp(); PERFORM grove.commit('night '||n,'p');
    cm := cm + extract(epoch FROM clock_timestamp()-t0)*1000;
  END LOOP;
  RAISE WARNING 'mean of $NIGHTS more commits   %ms', round(cm/$NIGHTS);
  INSERT INTO shas VALUES ('b', grove.resolve('main'));

  t0 := clock_timestamp();
  SELECT count(*) INTO c FROM grove.diff((SELECT sha FROM shas WHERE label='a'),(SELECT sha FROM shas WHERE label='b'));
  ...
  RAISE WARNING 'tree matches full rebuild  %',
    (grove.write_tree('r') = (SELECT root_hash FROM grove.trees WHERE commit_sha=grove.resolve('main') AND tbl='r'));
END \$\$;
SQL
)
RC=$?
...
RAW=$(psql "$D" -X -q -At -c "SELECT pg_total_relation_size('grove.nodes')")
if [ "${GC:-0}" != "1" ]; then
  ...
  echo "fsck problems              $(psql "$D" -X -q -At -c "SELECT count(*) FROM grove.fsck()")"
  echo "(GC=1 to also measure repack, which is slow)"
  exit 0
fi
echo "node store                 $(psql "$D" -X -q -At -c "SELECT pg_size_pretty($RAW::bigint)") against $(psql "$D" -X -q -At -c "SELECT pg_size_pretty(pg_total_relation_size('r'))") of data"
T0=$(date +%s); P=$(psql "$D" -X -q -At -c "SELECT grove.repack()"); T1=$(date +%s)
psql "$D" -X -q -c "VACUUM FULL grove.nodes" >/dev/null
B=$(psql "$D" -X -q -At -c "SELECT pg_total_relation_size('grove.nodes')")
