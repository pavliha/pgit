...
DSN="${GROVE_REAL_DSN:-postgresql://postgres:grove@${GROVE_HOST:-localhost:5460}/grove_realapp}"
...
if [ -n "$DUMP" ]; then
  ...
  psql "$ADMIN" -X -q -c "DROP DATABASE IF EXISTS grove_realapp" -c "CREATE DATABASE grove_realapp" >/dev/null 2>&1
...
qs "DO \$\$
    ...
    BEGIN
      FOR r IN
        ...
        ORDER BY c.relname
      LOOP
        PERFORM grove.track(r.t);
      END LOOP;
    END \$\$"

TRACKED=$(q "SELECT count(*) FROM grove.tracked")
...
qs "SELECT grove.commit('baseline of the real database','realworld')" >/dev/null
...
is "realworld: the baseline commit produced a tree for every tracked table" \
   "$(q "SELECT count(*) FROM grove.trees WHERE commit_sha = grove.resolve('main')")" "$TRACKED"

is "realworld: the working tree is clean straight after committing it" "$(q "SELECT grove.is_dirty()")" "f"

is "realworld: fsck finds nothing wrong" "$(q "SELECT count(*) FROM grove.fsck()")" "0"

is "realworld: a full rebuild of every table reproduces the recorded root hash" \
   "$(q "SELECT count(*) FROM grove.tracked t
         JOIN grove.trees r ON r.tbl = t.tbl::text AND r.commit_sha = grove.resolve('main')
         WHERE grove.write_tree(t.tbl) IS DISTINCT FROM r.root_hash")" "0"

echo "# a real pricing change on a branch"
qs "SELECT grove.branch('pricing')" >/dev/null
qs "SELECT grove.checkout('pricing')" >/dev/null
...
qs "SELECT grove.commit('raise active prices by 10 percent','pricing bot')" >/dev/null
...
is "realworld: the diff sees exactly the repriced rows" \
   "$(q "SELECT count(*) FROM grove.diff(grove.rev('pricing~1'), grove.resolve('pricing'))")" "$CHANGED"

is "realworld: and attributes every one of them to the products table" \
   "$(q "SELECT count(DISTINCT tbl) FROM grove.diff(grove.rev('pricing~1'), grove.resolve('pricing'))")" "1"

echo "# an unrelated translation change on main"
qs "SELECT grove.checkout('main')" >/dev/null
...
qs "SELECT grove.commit('review the georgian copy','editor')" >/dev/null
case "$TR" in (*[!0-9]*|"") echo "not ok - translation update failed: $TR" >&2; exit 1;; esac
echo "# retranslated $TR rows"

MERGED=$(qs "SELECT grove.merge('pricing')")
...
is "realworld: the merge kept both changes" \
   "$(q "SELECT count(*) FROM grove.diff(grove.rev('main~2'), grove.resolve('main'))")" "$((CHANGED + TR))"

is "realworld: fsck is still clean after the merge" "$(q "SELECT count(*) FROM grove.fsck()")" "0"
is "realworld: the merge left the working tree clean" "$(q "SELECT grove.is_dirty()")" "f"

echo "# two branches editing the same product price"
PID=$(q "SELECT id FROM products WHERE is_active ORDER BY id LIMIT 1")
qs "SELECT grove.branch('promo')" >/dev/null
qs "SELECT grove.checkout('promo')" >/dev/null
qs "UPDATE products SET price_amount = 111 WHERE id = $PID" >/dev/null
qs "SELECT grove.commit('promo price','marketing')" >/dev/null
qs "SELECT grove.checkout('main')" >/dev/null
qs "UPDATE products SET price_amount = 222 WHERE id = $PID" >/dev/null
qs "SELECT grove.commit('list price','finance')" >/dev/null

CONF=$(qs "SELECT grove.merge('promo')")
is "realworld: the same product edited on both sides is one conflict" "$CONF" "1"

MID=$(q "SELECT max(id) FROM grove.merges")
K=$(q "SELECT k FROM grove.conflicts WHERE merge_id = $MID LIMIT 1")
qs "SELECT grove.resolve_conflict($MID, 'products', '$K', 'theirs')" >/dev/null
qs "SELECT grove.merge_finish($MID)" >/dev/null
is "realworld: resolving to theirs takes the promo price" "$(q "SELECT price_amount FROM products WHERE id = $PID")" "111"
is "realworld: fsck is clean after a resolved conflict" "$(q "SELECT count(*) FROM grove.fsck()")" "0"
...
must "SELECT grove.branch('cleanup')" >/dev/null
must "SELECT grove.checkout('cleanup')" >/dev/null
...
must "SELECT grove.commit('retire a category and its products','ops')" >/dev/null
echo "# cleanup branch removed $GONE products and category $CID"

must "SELECT grove.checkout('main')" >/dev/null
NEWSKU="grove-realworld-probe"
...
must "SELECT grove.commit('new product in that category','merch')" >/dev/null
...
OUT=$(q "SELECT grove.merge('cleanup')")
...
is "realworld: and the working tree still matches the commit before it" "$(q "SELECT grove.is_dirty()")" "f"

echo "# blame on a real row"
is "realworld: blame reports the price the row actually holds now" \
   "$(q "SELECT value #>> '{}' FROM grove.blame('products', '$PID') WHERE col = 'price_amount'")" \
...
is "realworld: and blames it to a commit on this branch, not the branch we refused to merge" \
   "$(q "SELECT count(*) FROM grove.blame('products', '$PID') b
         WHERE b.col = 'price_amount'
           AND b.commit_sha IN (SELECT a FROM grove.ancestors(grove.resolve('main')))")" "1"

SIZE=$(q "SELECT pg_size_pretty(pg_total_relation_size('grove.nodes'))")
TBL=$(q "SELECT pg_size_pretty(sum(pg_total_relation_size(relid))::bigint) FROM pg_stat_user_tables WHERE relname NOT LIKE 'grove%'")
