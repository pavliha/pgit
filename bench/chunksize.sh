...
BEGIN
  t0 := clock_timestamp(); s := pgit.commit('base','b');
  ...
  t0 := clock_timestamp(); s := pgit.commit('k1000','b');
  ...
  t0 := clock_timestamp(); s := pgit.commit('k10','b');
  INSERT INTO res VALUES ('commit 10 rows', extract(epoch FROM clock_timestamp()-t0)*1000);
  INSERT INTO shas VALUES ('c', s);

  t0 := clock_timestamp();
  SELECT count(*) INTO n FROM pgit.diff((SELECT v FROM shas WHERE k='b'),(SELECT v FROM shas WHERE k='c'));
  INSERT INTO res VALUES ('diff 10 rows', extract(epoch FROM clock_timestamp()-t0)*1000);

  t0 := clock_timestamp();
  SELECT count(*) INTO n FROM pgit.diff((SELECT v FROM shas WHERE k='a'),(SELECT v FROM shas WHERE k='b'));
  ...
  FOR i IN 1..200 LOOP
    ...
    PERFORM pgit.commit('x'||i,'b');
  END LOOP;
  INSERT INTO res VALUES ('mean per commit over 200', extract(epoch FROM clock_timestamp()-t0)*1000/200);

  t0 := clock_timestamp();
  SELECT count(*) INTO n FROM pgit.diff((SELECT v FROM shas WHERE k='a'), pgit.resolve('main'));
...
SELECT 'r '||rpad('node storage',28)||lpad(pg_size_pretty(pg_total_relation_size('pgit.nodes')),10);
SELECT 'r '||rpad('node count',28)||lpad(count(*)::text,10) FROM pgit.nodes;
SELECT 'r '||rpad('tree depth',28)||lpad(pgit.node_level(root_hash)::text,10) FROM pgit.trees WHERE commit_sha=pgit.resolve('main') AND tbl='big';
EOF
  echo "=== chunk_target=$TARGET ==="
done
psql "$ADMIN" -X -q -c "DROP DATABASE IF EXISTS pgit_cs" >/dev/null
